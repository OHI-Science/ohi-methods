...
layers <- read.csv("https://raw.githubusercontent.com/OHI-Science/ohi-global/refs/heads/draft/metadata_documentation/layers_eez_base.csv")
targets <- read.csv("https://raw.githubusercontent.com/OHI-Science/ohi-global/refs/heads/draft/metadata_documentation/layers_eez_targets.csv")
...
The habitat subgoal measures the average condition of marine habitats present in each region that provide critical habitat for a broad range of species (mangroves, coral reefs, seagrass beds, kelp forests, salt marshes, sea ice edge, tidal flats, subtidal soft bottom, and beaches). This subgoal is considered a proxy for the condition of the broad suite of marine species.
...
| Beaches      | Conditon was determined from a 2018 report on global erosion rates (% of affected beaches * erosion rate * number of years since assessment), and scaled from 0-1.        | Sandy beach extent per oceanic region (vector based)| Sandy beach trend was calculated from changes in coastal area (km2) from 2010 to 2020, multiplied by year's since assessment, and applied to each country's area. Trend was rescaled to 0-1.                                                                                                            |
...
The status of trash pollution was estimated using globally-available plastic pollution data [@ncei_ncei_2025].
...
After remaining static since 2013 (changes across scenario years after 2013 are due to changes in pressures/resilience) [@unitednations2013faob], the economies subgoal was initially overhauled in 2025, with room for improvement during subsequent assessments.
