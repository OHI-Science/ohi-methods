...
```{r pressure matrix, eval = TRUE, echo=FALSE, warning=FALSE, message=FALSE, error=FALSE}
...
::: {.callout-note collapse=true}
# Table 5.3. Resilience categories and weight table
...
# new **Table 5.4. Resilience matrix**
...
resilience_matrix <- read.csv("https://raw.githubusercontent.com/OHI-Science/ohi-global/draft/eez/conf/resilience_matrix.csv") %>%
#  mutate(element = ifelse(is.na(element_name), element, element_name)) %>%
#  select(-element_name) %>%
  pivot_longer(cols=-c("goal", "element"),
